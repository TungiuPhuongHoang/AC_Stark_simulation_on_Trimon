...
def measure_loop_phase_with_detune_VZ(loop_id: str, dt: float = 0.25e-9) -> float:
...
def main():
    ...
    # Rabi cap sweep (MHz)
    caps_MHz = np.linspace(2, 60, 30)  # 4,6,...,40 MHz
    dphi_stark_calc = []
    dphi_meas_detVZ = []
    ...
    for cap in caps_MHz:
        ...
        print(f"cap={cap:5.1f} MHz  |  Δφ_stark(calc)={dphi_calc: .6f} rad   "
              f"Δφ_meas(detune-VZ)={dphi_meas: .6f} rad")

    # --- Plot ---
    plt.figure(figsize=(7.6, 4.6), dpi=160)
    plt.plot(caps_MHz, dphi_stark_calc, marker='o', label='Calculated Stark-only Δφ')
