...
def build_segment(
    ...
    # If the peak would exceed cap, stretch T
    ...
    # Optionally update software frame with DETUNE ONLY (never Stark)
    if VZ_COMPENSATE:
        for q in ('A', 'B', 'C'):
...
def run_chain(
    ...
    if plot:
        ...
        for idx, x in enumerate(seg_end_ns, start=1):
            ...
            # annotate segment number at the top
...
if __name__ == "__main__":
    ...
    tB, sB, psiB = run_chain(psi000, segs_B, title="Loop-B (3 pulses)",
                             apply_vz_to_state=True)  # set True to apply detune-VZ during run
